...
async function xxhash() {
	...
	function h32(str, seed = defaultSeed) {
		...
		return forceUnsigned32(xxh32(0, encoder.encodeInto(str, memory).written, seed));
	}

	function h64(str, seed = defaultBigSeed) {
		growMemory(str.length * 3, 0);
		return forceUnsigned64(xxh64(0, encoder.encodeInto(str, memory).written, seed));
	}
	...
	return {
		...
		create32(seed = defaultSeed) {
			return create(XXH32_STATE_SIZE_BYTES, seed, init32, update32, digest32, forceUnsigned32);
		},
		h64,
		...
		create64(seed = defaultBigSeed) {
			return create(XXH64_STATE_SIZE_BYTES, seed, init64, update64, digest64, forceUnsigned64);
		},
	};
}
